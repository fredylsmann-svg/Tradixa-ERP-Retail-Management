...
export default function PublicGRNManagerSign() {
  const { id } = useParams();
  ...
  const handleSign = async (signatureData) => {
    ...
    try {
      const historyEntry = {
        time_wib: ts,
        activity: 'Persetujuan Manajer (GRN)',
        detail: `Goods receipt notes disetujui secara digital oleh Manajer Gudang`.toUpperCase(),
        type: 'sign'
      };
      await api.entities.GoodsReceipt.update(id, {
        warehouse_manager_signature: signatureData,
        manager_signed_at: now.toISOString(),
        warehouse_manager_name: 'Manager Gudang',
        approval_history: [...(grn.approval_history || []), historyEntry]
      });
      setGrn(prev => ({ ...prev, warehouse_manager_signature: signatureData }));
      setIsSuccess(true);
    } catch {
    ...
    <div className="min-h-screen bg-emerald-50 flex items-center justify-center p-6">
      <Card className="max-w-lg w-full p-12 text-center space-y-6">
        ...
        <div className="bg-slate-50 rounded-2xl p-6 border-2 border-dashed border-slate-200 relative overflow-hidden">
          <img src={grn.warehouse_manager_signature} alt="Tanda Tangan" className="h-28 object-contain mx-auto dark:invert dark:brightness-150" />
          <div className="absolute top-2 right-2 bg-emerald-500 text-white text-[10px] font-bold px-2 py-0.5 rounded">APPROVED</div>
        </div>
        <p className="font-bold text-slate-800 text-lg uppercase">{grn.warehouse_manager_name || 'Manager Gudang'}</p>
  ...
  // ---------- MAIN VIEW ----------
  const alreadySigned = !!grn.warehouse_manager_signature;
        ...
        {alreadySigned ? (
          <Card className="border-none shadow-sm rounded-3xl bg-emerald-50 p-8 text-center space-y-4">
            ...
            <div className="bg-white rounded-2xl p-4 border border-dashed border-emerald-200 inline-block mx-auto">
              <img src={grn.warehouse_manager_signature} alt="TTD" className="h-24 object-contain mx-auto dark:invert dark:brightness-150" />
            </div>
            <Button
              onClick={() => {
                const htmlContent = `
          ...
          <Card className="border-none shadow-xl rounded-3xl bg-white p-8 overflow-hidden relative">
            <div className="max-w-md mx-auto space-y-6 text-center">
              <div>
                <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">Persetujuan Manajer Gudang</h2>
                <p className="text-sm text-slate-500 mt-2">Dengan menandatangani di bawah ini, Anda menyatakan telah menyetujui penerimaan barang sesuai dengan rincian GRN ini.</p>
              </div>
              <div className="bg-white p-4 rounded-3xl shadow-inner border border-slate-100">
                <SignaturePad
                  onSave={handleSign}
                  title={`Tanda Tangan Manajer Gudang`}
                />
              </div>

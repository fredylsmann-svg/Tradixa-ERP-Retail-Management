...
import { Eye, DollarSign, Wallet, Loader2, Plus, Camera, Sparkles, AlertTriangle, Info } from 'lucide-react';
...
export default function Receivables({ store }) {
  ...
  const [isGeneratingMayar, setIsGeneratingMayar] = useState(false);
  ...
  const [formData, setFormData] = useState({ customer_id: '', amount: '', due_date: '', notes: '' });
  ...
  const loadReceivables = async () => {
    const [data, paymentsData] = await Promise.all([
      api.entities.Receivable.filter({ store_id: store.id }, '-created_date'),
      api.entities.InvoicePayment.filter({ store_id: store.id, invoice_type: 'Receivable' })
    ]);
    setReceivables(data);
  ...
  const loadCustomers = async () => {
    const data = await api.entities.Customer.filter({ store_id: store.id }, 'name');
    setCustomers(data);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const invoiceNumber = `RCV-${Date.now()}`;
    ...
    // --- JOURNAL ENTRY: Pencatatan Piutang Baru ---
    const arJournal = await api.entities.JournalEntry.create({
      store_id: store.id,
      transaction_id: invoiceNumber,
      date: new Date().toISOString(),
      description: `Piutang Baru - ${selectedCustomer?.name} (${invoiceNumber})`,
      type: 'Receivable',
      status: 'Draft',
      total_debit: amount,
      total_credit: amount,
    ...
    await Promise.all([
      api.entities.JournalLine.create({
        journal_id: arJournal.id,
        account_name: 'Piutang Usaha',
        description: `Piutang baru dari ${selectedCustomer?.name}`,
        debit: amount,
        credit: 0
      }),
      api.entities.JournalLine.create({
        ...
        description: `Pendapatan piutang - ${selectedCustomer?.name}`,
        debit: 0,
        credit: amount
      })
    ]);
    ...
    setIsSaving(false);
    setShowForm(false);
    setFormData({ customer_id: '', amount: '', due_date: '', notes: '' });
    loadReceivables();
  };

  const handleGenerateMayar = async () => {
    if (!viewingItem) return;
    setIsGeneratingMayar(true);
    try {
      // 1. Fetch real customer email and phone from customer master
      const { data: customerData } = await supabase
        .from('customers')
        .select('email, phone')
        .eq('id', viewingItem.customer_id)
        .single();

      // 2. Invoke Mayar Edge Function
      const { data, error } = await supabase.functions.invoke('mayar-create-link', {
        body: {
          receivable_id: viewingItem.id,
          amount: viewingItem.remaining_amount,
  ...
  const handlePayment = async () => {
    ...
    // 1. Update Receivable
    await api.entities.Receivable.update(paymentDialog.id, {
    ...
    if (excessAmount > 0 || depositUsed > 0) {
      ...
      await api.entities.Customer.update(paymentDialog.customer_id, { advance_balance: newAdvanceBalance });
    }
    ...
    if (paymentAmount > 0) {
      ...
      if (paymentMethod === 'Bank') {
        ...
        await api.entities.BankTransaction.create({
          ...
          description: `Pelunasan Piutang - ${paymentDialog.customer_name} (${paymentDialog.invoice_number})`,
          reference,
          status: 'Approved',
      ...
      const arAccount = coaAccounts.find(a => a.name.includes('Piutang Usaha'))?.name || 'Piutang Usaha';
      ...
      await Promise.all([
        ...
        api.entities.JournalLine.create({
          journal_id: journal.id,
          account_name: arAccount,
          description: `Pengurangan Piutang - ${paymentDialog.customer_name}`,
          debit: 0,
    ...
    if (depositUsed > 0) {
      ...
      const depositJournal = await api.entities.JournalEntry.create({
        ...
        date: new Date().toISOString(),
        description: `Pembayaran Deposit Pelanggan - ${paymentDialog.customer_name} (${paymentDialog.invoice_number})`,
        type: 'Receipt',
        status: 'Draft',
      ...
      const coaAccounts = await api.entities.COA.filter({ store_id: store.id });
      const arAccount = coaAccounts.find(a => a.name.includes('Piutang Usaha'))?.name || 'Piutang Usaha';
      const depositAccount = coaAccounts.find(a => a.name.includes('Uang Muka'))?.name || 'Uang Muka Pelanggan';

      await Promise.all([
        api.entities.JournalLine.create({
          journal_id: depositJournal.id,
          account_name: depositAccount,
          description: `Pemakaian Deposit Pelanggan - ${paymentDialog.customer_name}`,
          debit: depositUsed,
          credit: 0
        }),
        api.entities.JournalLine.create({
          journal_id: depositJournal.id,
          account_name: arAccount,
          description: `Pengurangan Piutang via Deposit - ${paymentDialog.customer_name}`,
          debit: 0,
          credit: depositUsed
        })
      ]);
  ...
  const totalReceivables = receivables.filter(r => r.status !== 'Paid').reduce((sum, r) => sum + (r.remaining_amount || 0), 0);

  return (
    <div className="space-y-6">
      <PageHeader
        title="Account Receivables"
        subtitle="Manajemen tagihan kepada pelanggan"
        icon={Wallet}
        actions={
          <>
            <ExportToolbar 
      ...
      <Card className="bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl border-none shadow-xl">
        <CardContent className="p-8">
          <p className="text-base font-medium text-blue-100 tracking-wide mb-1">Total Piutang Belum Lunas</p>
          <p className="text-4xl font-black mt-2">
            <AnimatedNumber value={totalReceivables} prefix="Rp " />
          </p>
        </CardContent>
      </Card>
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={9} className="text-center py-20 text-slate-400">
                    ...
                    <p className="font-bold">Belum ada piutang terdaftar</p>
                  </TableCell>
                </TableRow>
              ) : (
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="rounded-xl">
          <DialogHeader><DialogTitle className="font-black text-xl">Tambah Piutang Baru</DialogTitle></DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 pt-4">
            <div className="space-y-1.5">
              <Label className="text-xs font-bold uppercase text-slate-400">Pilih Pelanggan *</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold uppercase text-slate-400">Jumlah Piutang *</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold uppercase text-slate-400">Jatuh Tempo *</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold uppercase text-slate-400">Catatan Internal</Label>
            ...
            <DialogFooter className="pt-6">
              ...
              <Button type="submit" disabled={isSaving} className="bg-blue-600 hover:bg-blue-700 text-white font-bold h-12 px-8 rounded-xl">
                {isSaving && <Loader2 className="w-4 h-4 animate-spin mr-2" />}Simpan Piutang
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog open={!!paymentDialog} onOpenChange={() => setPaymentDialog(null)}>
        <DialogContent className="rounded-xl border-none p-0 max-h-[90vh] flex flex-col">
          <DialogHeader className="px-8 pt-8 pb-0">
            <DialogTitle className="text-2xl font-black text-slate-900">Terima Pembayaran</DialogTitle>
          </DialogHeader>
          <div className="space-y-6 pt-4 px-8 pb-8 overflow-y-auto">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Sisa Piutang</p>
              <p className="text-2xl font-black text-slate-900">Rp {formatCurrency(paymentDialog?.remaining_amount || 0)}</p>
            </div>

            {/* Step 1: Deposit (if available) */}
            {(() => {
              const customerBalance = customers.find(c => c.id === paymentDialog?.customer_id)?.advance_balance || 0;
              if (customerBalance <= 0) return null;
              const remaining = paymentDialog?.remaining_amount || 0;
              const maxDeposit = Math.min(customerBalance, remaining);
              return (
                <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <Label htmlFor="useDeposit" className="text-xs font-bold text-emerald-900 cursor-pointer">Gunakan Saldo Titipan</Label>
                      <p className="text-[10px] text-emerald-700">Tersedia: Rp {formatCurrency(customerBalance)}</p>
                    </div>
                  </div>
                  {useDeposit && (
                    <div className="space-y-2">
                      <Label className="text-[10px] font-bold uppercase text-emerald-800">Jumlah Deposit yang Digunakan</Label>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 font-bold text-emerald-500 text-sm">Rp</span>
                        <NumberInput value={depositAmount} onChange={(e) => { const val = Math.min(Number(e.target.value), maxDeposit); setDepositAmount(val); setPaymentAmount(Math.max(0, remaining - val)); }} className="h-10 pl-10 rounded-xl bg-white border border-emerald-200 font-black text-sm text-emerald-900" />
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <button type="button" onClick={() => { setDepositAmount(maxDeposit); setPaymentAmount(Math.max(0, remaining - maxDeposit)); }} className="text-[9px] font-bold text-emerald-700 bg-emerald-200/70 hover:bg-emerald-200 px-2.5 py-1 rounded-lg transition-colors">Maks (Rp {formatCurrency(maxDeposit)})</button>
                        <button type="button" onClick={() => { const half = Math.round(maxDeposit / 2); setDepositAmount(half); setPaymentAmount(Math.max(0, remaining - half)); }} className="text-[9px] font-bold text-emerald-700 bg-emerald-200/70 hover:bg-emerald-200 px-2.5 py-1 rounded-lg transition-colors">50%</button>
                        <button type="button" onClick={() => { setDepositAmount(0); setPaymentAmount(remaining); }} className="text-[9px] font-bold text-slate-500 bg-slate-100 hover:bg-slate-200 px-2.5 py-1 rounded-lg transition-colors">Reset</button>
                      </div>
                      {Number(depositAmount) > 0 && Number(depositAmount) < remaining && (
                        <div className="pt-2">
                          <div className="flex justify-between items-center mb-1.5">
                            <Label className="text-[9px] font-black uppercase tracking-wider text-emerald-800/70">Opsi Pembayaran Parsial</Label>
                            <Popover>
                              ...
                              <PopoverContent className="w-64 bg-slate-900 text-white text-[10px] p-3 rounded-xl shadow-2xl border-slate-700" side="top" align="end" sideOffset={5}>
                                ...
                                <ul className="list-disc pl-3 space-y-1.5 text-slate-300 leading-relaxed">
                                  <li><strong className="text-white">Lunasi Sisa:</strong> Menerima kekurangan dana menggunakan Cash/Bank. Status piutang akan menjadi <strong className="text-emerald-400">Lunas</strong>.</li>
                                  <li><strong className="text-white">Hanya Deposit:</strong> Hanya menggunakan deposit tanpa menerima uang cash tambahan. Status piutang akan menjadi <strong className="text-amber-400">Sebagian</strong>.</li>
                                </ul>
                              </PopoverContent>
                            </Popover>
                          </div>
            ...
            {useDeposit && Number(depositAmount) >= (paymentDialog?.remaining_amount || 0) && (
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-emerald-900">Seluruh tagihan ditutup dari deposit</p>
                  <p className="text-[10px] text-emerald-700">Tidak perlu pembayaran cash/bank tambahan</p>
                </div>
              </div>
            )}
            ...
            {(!useDeposit || Number(paymentAmount) > 0) && (
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-slate-400">Rp</span>
                  <NumberInput value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} className="h-12 pl-12 rounded-xl bg-slate-50 border-none font-black text-lg" />
                </div>
              </div>
            )}
            ...
            {Number(paymentAmount) > 0 && (
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold uppercase text-slate-400">{useDeposit ? 'Metode untuk Sisa' : 'Metode Pembayaran'}</Label>
                ...
                {paymentMethod === 'Bank' && (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-bold uppercase text-slate-400">Pilih Rekening</Label>
            ...
            {useDeposit && (
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1.5">
                ...
                <div className="flex justify-between text-xs">
                  <span className="text-emerald-700 font-bold">Dari Deposit Pelanggan</span>
                  <span className="font-black text-emerald-700">Rp {formatCurrency(depositAmount)}</span>
                </div>
                {Number(paymentAmount) > 0 && (
                  <div className="flex justify-between text-xs">
                ...
                <div className="flex justify-between text-xs border-t border-slate-200 pt-1.5 mt-1">
                  <span className="text-slate-900 font-black">Total Diterima</span>
          ...
          <DialogHeader>
            <div className="flex justify-between items-center pr-8">
              ...
              {viewingItem && !viewingItem.payment_link && viewingItem.status !== 'Paid' && (
                ...
                >
                  {isGeneratingMayar ? 'Loading...' : !store?.mayar_api_key ? '⚡ Mayar Belum Aktif' : '⚡ Buat Link Mayar'}
                </Button>
              )}
            </div>
          </DialogHeader>
          {viewingItem && (
            <div className="space-y-5 pt-4">
              <div className="grid grid-cols-2 gap-y-5 gap-x-4">
                ...
                <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Customer</p><p className="font-bold text-slate-800">{viewingItem.customer_name}</p></div>
                <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Piutang</p><p className="font-black text-slate-900">Rp {formatCurrency(viewingItem.amount)}</p></div>
                ...
                <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sisa Piutang</p><p className="font-black text-blue-600">Rp {formatCurrency(viewingItem.remaining_amount)}</p></div>
              ...
              {viewingItem.payment_link && (
                <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100 flex flex-col md:flex-row items-center justify-between gap-4 mt-2">
                  <div>
                    <h4 className="text-sm font-black text-indigo-900 mb-1">🔗 Mayar Payment Link</h4>
                    <p className="text-[10px] font-medium text-indigo-700">Kirimkan link ini ke pelanggan.</p>
                  </div>
                  <div className="flex items-center gap-2 w-full md:w-auto">
                    <Input readOnly value={viewingItem.payment_link} className="bg-white border-indigo-200 text-xs font-medium h-8 w-full md:w-48" />
                    <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg h-8 px-3 text-xs" onClick={() => {
                      navigator.clipboard.writeText(viewingItem.payment_link);
                      toast({ title: 'Sukses', description: 'Link disalin ke clipboard!' });
                    }}>Copy</Button>
                  </div>
                </div>
              )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="border border-slate-200 rounded-xl overflow-hidden">
                      ...
                      {depositPayments.map((p, i) => (
                        <div key={`dp-${i}`} className="flex justify-between items-center px-4 py-3 bg-emerald-50 border-b border-emerald-100 last:border-b-0">
                          <div>
                            <p className="text-xs font-bold text-emerald-800">Saldo Deposit Pelanggan</p>
                            <p className="text-[10px] text-emerald-500">{p.payment_date} • Ref: {p.reference}</p>
                          </div>
                          <p className="text-sm font-black text-emerald-700">Rp {formatCurrency(p.amount)}</p>
                        </div>
                      ))}
                    ...
                    {overpayment > 0 && (
                      <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                        <div className="flex justify-between items-center">
                          <div>
                            <p className="text-xs font-bold text-emerald-900">Kelebihan Bayar</p>
                            <p className="text-[10px] text-emerald-700 font-medium">Otomatis masuk ke Saldo Titipan Pelanggan</p>
                          </div>

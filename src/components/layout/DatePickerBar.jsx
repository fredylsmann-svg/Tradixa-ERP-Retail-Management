...
function MiniCalendar({ value, onChange, onClose }) {
  ...
  return (
    <div ref={ref} className="absolute top-full right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-200 p-4 z-[100] w-[320px] select-none" onClick={(e) => e.stopPropagation()}>
      {/* Header */}
        ...
        {cells.map((cell, idx) => {
          ...
          return (
            <button
              ...
              className={`
                w-9 h-9 mx-auto rounded-lg text-sm font-medium transition-all
...
export default function DatePickerBar() {
  ...
  return (
    <div className="flex items-center gap-2 bg-gradient-to-r from-slate-50 to-blue-50/30 border border-slate-200 rounded-xl px-3 py-1.5 relative">
      <button onClick={goPrev} className="p-1 rounded-md hover:bg-white transition-colors" title="Hari sebelumnya">
        <ChevronLeft className="w-4 h-4 text-slate-500" />
      </button>
      
      <button
        onClick={() => setShowCalendar(!showCalendar)}
        className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-white transition-colors"
      >
        <Calendar className="w-4 h-4 text-blue-600" />
        <span className="text-sm font-semibold text-slate-700 whitespace-nowrap hidden md:inline">
          {formattedDate}
        </span>
        <span className="text-sm font-semibold text-slate-700 whitespace-nowrap md:hidden">
          {new Date(selectedDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
        </span>
      </button>

      <button onClick={goNext} className="p-1 rounded-md hover:bg-white transition-colors" title="Hari berikutnya">
      ...
      {showCalendar && (
        <MiniCalendar
          value={selectedDate}
          onChange={(d) => { if (d) setSelectedDate(d); else goToday(); }}
